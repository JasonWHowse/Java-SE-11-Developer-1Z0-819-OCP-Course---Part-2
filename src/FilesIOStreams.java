...
public class FilesIOStreams {
    ...
    private static void readingAFile(Path testFile) {
        ...
        } catch (IOException io) {//try (BufferedReader inStream =
        ...
        } catch (IOException io) {//try (InputStream in = Files.newInputStream(testFile);
            System.out.println(io);
        }//catch (IOException io) {
        ...
        } catch (IOException io) {//try (BufferedReader reader = Files.newBufferedReader(testFile)) {
            System.out.println(io);
        }//catch (IOException io) {
    }//private static void readingAFile(Path testFile) {
    ...
    private static void writingAFile(Path testFile) throws IOException {
        ...
        try (BufferedWriter writer =
            ...
            writer.write(line);
            writer.newLine();
        } catch (IOException io) {//try (BufferedWriter writer =
        ...
        } catch (IOException io) {//try (OutputStream out = new BufferedOutputStream(
            System.err.println(io);
        }//catch (IOException io) {
        Files.readAllLines(testFile)
                .forEach((s) -> System.out.println("\t" + s));
        ...
        try (BufferedWriter out = Files.newBufferedWriter(testFile)) {
            out.write(line);
            out.newLine();
        } catch (IOException io) {//try (BufferedWriter out = Files.newBufferedWriter(testFile)) {
            System.out.println(io);
        }//catch (IOException io) {
        Files.readAllLines(testFile)
                .forEach((s) -> System.out.println("\t" + s));
